...
The program first opens a ".pyMusic" file and remove all the
whitespace characters (like tabs, spacebars, line feeds).  It then
finds a JSON table using "Regular Expressions" and set defaults for
this program to output the score.  After that, it reads through the
score and reads each notes in-between two forward slashes.  The note
contains information on how to play the note and must be splitted up
into individual parts.  Each part tells important information on how
to play and then runs the playing function.

The pitches you hear are based off of how frequent you hear 1
pitch. The A440 (4th octave A) is 440 Hz meaning that the pitch you
hear is actually played 440 times a second. Each half-step adds or
subtracts 2 ^ (1/12) to a multiplier (A440 having 2 ^ (0/12) or simply
1) and multiplys it by 440 to get how fast does the buzzer fluctuate.
...
The JSON table basically indicates the 4 basic settings of PyMusic:
 ...
 - "key" takes a Major scale (indicated in "scaleListing") and defaults
    to the key of "C". This is used for automatically setting the notes
    to a flat, natural, or sharp.
 - "bpm" takes a number (never tested with a float) which stands for
    the "beats per minute." Defaults to 120 BPM.
 - "time_signature" takes pref. an int to decide which note is 1 beat
    (i.e. set it to 4 to make quarter notes equal a beat).
 - "base_octave" takes an int to automatically set notes to a certain
...
Omitting the symbol to how the note is played defaults to playing it
normally. You can find a list in "styleTab" in this code.

Omitted the octave defaults to the "base_octave" set earlier in the
code.

The Note contains:
 - The literal letter of the note.

This indicates what note should be played. You can replace the note
with a "R" to rest instead. "R" supports length of a note.

The Suffix contains:
 - Flat, Natural, Sharp accidentals
 - How long the note is played for

Flat, Natural, Sharps override the default accidental for the
note. Flats are indicated as "b", Naturals are indicated as "z", and
Sharps are indicated as "#".

How long the note is played is a number (supports float) that defaults
to 1 beat. The number corresponds to the name of the note (i.e. 4 is a
quarter note, 1 is a whole note, 2 is a half note, 32 is a
thirty-second note, 3 is a dotted quarter note)

(Note: Commenting out code uses parentheses. If I wanted to make a
comment in the middle of the score (or even a note), you can simply
do:
...
I know this isn't much of a tutorial and it's kinda wordy and probably
...
scaleListing = {'C':  ['z', sharpFlatOrder[:]],
                'G':  ['#', sharpFlatOrder[-1]],
                'D':  ['#', sharpFlatOrder[-2:]],
                'A':  ['#', sharpFlatOrder[-3:]],
                'E':  ['#', sharpFlatOrder[-4:]],
